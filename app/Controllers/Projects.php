...
class Projects extends Security_Controller {
    ...
    private function can_delete_projects($project = false) {
        ...
        } else if (get_setting("client_can_edit_projects") && $project !== false  && $this->login_user->id == $project->created_by) {
            return true;
        }
    }

    private function can_add_remove_project_members($project_id) {
        if ($this->login_user->user_type == "staff") {
            ...
            if($project && $project->created_by == $this->login_user->id) {
                return true;
            }
        }
    }
    ...
    private function can_edit_tasks($project_id = 0) {
        if ($this->login_user->user_type == "staff") {
            if ($this->can_manage_all_projects()) {
                return true;
    ...
    private function can_delete_tasks($project_id = 0) {
        if ($this->login_user->user_type == "staff") {
            if ($this->can_manage_all_projects()) {
                return true;
    ...
    private function can_create_milestones($project_id) {
        if ($this->login_user->user_type == "staff") {
            if ($this->can_manage_all_projects()) {
                return true;
    ...
    private function can_delete_milestones($project_id) {
        if ($this->login_user->user_type == "staff") {
            if ($this->can_manage_all_projects()) {
                return true;
    ...
    function delete() {
        ...
        $id = $this->request->getPost('id');
        $project = $this->Projects_model->get_details(array('id'=>$id))->getRow();
        if (!$this->can_delete_projects($project)) {
            app_redirect("forbidden");
        }
    ...
    function projects_list_data_of_client($client_id = 0, $dpt_id = '') {
    ...
    function overview($project_id) {
        ...
        $view_data["can_add_remove_project_members"] = $this->can_add_remove_project_members($project_id);
    ...
    function overview_for_client($project_id) {
        if ($this->login_user->user_type === "client") {
            ...
            if($view_data['can_add_remove_project_members']) { 
                $view_data['can_access_clients'] = true;
            }
    ...
    function project_member_modal_form() {
        ...
        if (!$this->can_add_remove_project_members($project_id)) {
            app_redirect("forbidden");
        }
        ...
        if ($add_user_type == "client_contacts") {
            $project = $this->Projects_model->get_details(array('id'=>$project_id))->getRow();
            if (!$this->can_access_clients() && $project->created_by != $this->login_user->id) {
                app_redirect("forbidden");
            }
    ...
    function delete_project_member() {
        ...
        if (!$this->can_add_remove_project_members($project_member_info->project_id)) {
            app_redirect("forbidden");
        }
    ...
    private function _make_project_member_row($data, $can_send_message_to_client = false) {
        ...
        if ($this->can_add_remove_project_members($data->project_id)) {
    ...
    function milestones($project_id) {
        ...
        $view_data["can_create_milestones"] = $this->can_create_milestones($project_id);
        $view_data["can_edit_milestones"] = $this->can_edit_milestones($project_id);
        $view_data["can_delete_milestones"] = $this->can_delete_milestones($project_id);
    ...
    function milestone_modal_form() {
        ...
        if ($id) {
            ...
            if (!$this->can_create_milestones($project_id)) {
                app_redirect("forbidden");
            }
        }
    ...
    function delete_milestone() {
        ...
        if (!$this->can_delete_milestones($info->project_id)) {
            app_redirect("forbidden");
        }
    ...
    private function _make_milestone_row($data) {
        ...
        $optoins = "";
        if ($this->can_edit_milestones($data->project_id)) {
        ...
        if ($this->can_delete_milestones($data->project_id)) {
    ...
    function tasks($project_id) {
        ...
        $view_data['can_edit_tasks'] = $this->can_edit_tasks($project_id);
        $view_data['can_delete_tasks'] = $this->can_delete_tasks($project_id);
    ...
    function can_edit_task_of_the_project($project_id = 0) {
        if ($project_id) {
            ...
            if ($this->can_edit_tasks($project_id)) {
                echo json_encode(array("success" => true));
            } else {
    ...
    function task_view($task_id = 0) {
        ...
        $view_data['can_edit_tasks'] = $this->can_edit_tasks($model_info->project_id);
    ...
    function task_modal_form() {
        ...
        $view_data['show_assign_to_dropdown'] = true;
        if ($this->login_user->user_type == "client" && $project->created_by != $this->login_user->id) {
            $view_data['show_assign_to_dropdown'] = false;
        } else {
    ...
    private function get_all_related_data_of_project($project_id, $collaborators = "", $task_labels = "") {
        ...
        if ($project_id) {
            ...
            $can_access = $this->can_access_clients();
            $project = $this->Projects_model->get_details(array("id" => $project_id))->getRow();
            if($project->created_by == $this->login_user->id) {
                $can_access = true;
            }

            $project_members = $this->Project_members_model->get_project_members_dropdown_list($project_id, "", $can_access)->getResult();
            $project_members_dropdown = array(array("id" => "", "text" => "-"));
    ...
    private function _make_sub_task_row($data, $return_type = "row") {
        ...
        $status = "";
        if ($this->can_edit_tasks($data->project_id)) {
    ...
    function update_task_info($id = 0, $data_field = "") {
        if ($id) {
            ...
            if (!$this->can_edit_tasks($task_info->project_id)) {
                app_redirect("forbidden");
            }
    ...
    function save_task_sort_and_status() {
        ...
        if (($this->login_user->user_type == "staff" && !can_edit_this_task_status($task_info->assigned_to)) || ($this->login_user->user_type == "client" && !$this->can_edit_tasks($project_id))) {
            app_redirect("forbidden");
        }
    ...
    function delete_task() {
        ...
        if (!$this->can_delete_tasks($info->project_id)) {
            app_redirect("forbidden");
        }
    ...
    private function _make_task_row($data, $custom_fields) {
        ...
        if ($this->can_delete_tasks($data->project_id)) {
