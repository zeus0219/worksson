...
class Team_members extends Security_Controller {
        ...
        foreach ($email_array as $email) {
            $verification_data = array(
                ...
                "params" => serialize(array(
                    "email" => $email,
                    "type" => $this->login_user->user_type,
                    "department_id" => $this->Departments_user_model->get_one($this->login_user->id)->department_id,
    ...
    function view($id = 0, $tab = "") {
        if ($id * 1) {
            ...
            if (!$this->can_view_team_members_list() && $this->login_user->id != $id && ($this->login_user->user_type == 'staff' || !$can_view)) {
                app_redirect("forbidden");
            }
