...
class Todo extends Security_Controller {
    ...
    private function _make_row($data, $is_manager = false) {
        ...
        $action = modal_anchor(get_uri("todo/modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit'), "data-post-id" => $data->id));
        if($is_manager || $this->login_user->user_type == 'staff') {
            $action .= js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("todo/delete"), "data-action" => "delete"));
        }
        return array(
            $status_class,
